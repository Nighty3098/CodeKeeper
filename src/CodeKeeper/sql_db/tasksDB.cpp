...
void MainWindow::updateTaskData(QString *task, QString *status, QString *cT) {
  ...
  if (!query.exec("UPDATE tasks SET task = '" + taskText[0] +
...
void MainWindow::updateTaskStatus(QString *task, QString *status, QString *cT) {
  ...
  if (!query.exec("UPDATE tasks SET task = '" + taskText[0] + "', status = '" +
                  status + "' WHERE createdTime = '" + cT + "'")) {
    qDebug() << query.lastError();
  }
}
...
void MainWindow::saveTaskToDB(QString *task, QString *status) {
  ...
  if (!query.exec("INSERT INTO tasks (task, status, createdTime) VALUES('" +
                  taskText[0] + "', '" + status + "', '" + taskText[1] +
                  "');")) {
    qDebug() << query.lastError();
  } else {
